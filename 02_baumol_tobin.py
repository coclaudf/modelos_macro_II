...
if verificar_autenticacion():
    ...
    *Desarrollado para la cátedra de Macroeconomía II (Basado en la notación de Sachs & Larraín). 
    ...
    st.sidebar.subheader("⚙️ Estado Inicial (Línea Base / Negro)")
    ...
    st.sidebar.subheader("⚡ Shock de Mercado / Innovación (Color)")
    ...
    # -------------------------------------------------------------------------
    # COLUMNA 1: MINIMIZACIÓN DE COSTOS (ESTÁTICA CON COMPARATIVA)
    # -------------------------------------------------------------------------
    with col_g1:
        ...
        # CURVAS INICIALES (NEGRO / GRIS REFERENCIA)
        costo_trans0 = b0 * N_grid
        costo_oport0 = i0 * (Q / (2 * N_grid))
        costo_total0 = costo_trans0 + costo_oport0

        if hay_shock:
            fig_costos.add_trace(go.Scatter(
                x=N_grid, y=costo_total0, name="CT₀ (Inicial - Negro)",
                line=dict(color='black', width=2, dash='dash')
            ))
            fig_costos.add_trace(go.Scatter(
                x=[N0_opt], y=[CT0_opt], mode='markers+text',
                text=['N₀*'], textposition='top center',
                marker=dict(color='black', size=10, symbol='square'), name="Punto Óptimo N₀*"
            ))

        # CURVAS POST-SHOCK (DISTINCIÓN DE COLOR)
        costo_trans1 = b1 * N_grid
        costo_oport1 = i1 * (Q / (2 * N_grid))
        costo_total1 = costo_trans1 + costo_oport1

        fig_costos.add_trace(go.Scatter(
            x=N_grid, y=costo_total1, name="CT₁ (Post-Shock)" if hay_shock else "Costo Total CT(N)",
            line=dict(color='blue', width=3)
        ))
        
        fig_costos.add_trace(go.Scatter(
            x=N_grid, y=costo_trans1, name="Costo Transaccional (b₁·N)",
            line=dict(color='orange', width=2)
        ))

        fig_costos.add_trace(go.Scatter(
            x=N_grid, y=costo_oport1, name="Costo Oportunidad (i₁·Q/2N)",
        ...
        fig_costos.add_trace(go.Scatter(
            ...
            marker=dict(color='blue', size=12, symbol='star'), name="Punto Óptimo N₁*"
        ))
    ...
    with col_g2:
        ...
        # Diente de Sierra Inicial (Si hay shock, se grafica en gris)
        if hay_shock:
            t_dias0, m_p_dias0 = generar_diente_sierra(Q, N0_opt, dias=30)
        ...
        fig_saw.update_layout(
            ...
            yaxis=dict(range=[0, max(Q / N0_opt, Q / N1_opt) * 1.25]),
            legend=dict(yanchor="top", y=0.99, xanchor="right", x=0.99, bgcolor="rgba(255,255,255,0.7)"),
            margin=dict(l=20, r=20, t=20, b=20), height=450
        )
        st.plotly_chart(fig_saw, use_container_width=True)

    # --- RECUADRO PEDAGÓGICO DE SÍNTESIS (FORMATO CLEAN EN MARKDOWN) ---
    costo_trans_val = b1 * N1_opt
    costo_oport_val = i1 * (Q / (2 * N1_opt))
    ...
    1. **Igualdad en el Óptimo:** En el punto mínimo $N^* = {N1_opt:.2f}$, el costo transaccional ($b \cdot N$) de **${costo_trans_val:.2f}** es exactamente igual al costo de oportunidad ($i \cdot \\frac{{Q}}{{2N}}$) de **${costo_oport_val:.2f}**.
