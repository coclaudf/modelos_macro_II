...
st.markdown("""
    <style>
    /* 1. Ocultar menú de Streamlit */
    #MainMenu { visibility: hidden !important; }
    footer { visibility: hidden !important; }
    header { visibility: hidden !important; }
    div[data-testid="stHeader"] { display: none !important; }

    /* 2. Fondo blanco puro universal */
    .stApp, [data-testid="stAppViewContainer"] {
        background-color: #FFFFFF !important;
        color: #0F172A !important;
    }
    
    /* 3. Barra lateral en gris muy suave con bordes oscuros */
    section[data-testid="stSidebar"] {
        background-color: #F8FAFC !important;
        border-right: 2px solid #CBD5E1 !important;
    }
    
    /* 4. Forzado de texto oscuro general */
    .stApp p, .stApp span, .stApp label, .stApp h1, .stApp h2, .stApp h3, .stApp h4, .stApp h5, .stApp h6 {
        color: #0F172A !important;
    }

    /* 5. BLINDAJE UNIFORME DE CAJAS ST.SUCCESS (DIAGNÓSTICO) */
    ...
    /* 6. BLINDAJE UNIFORME DE CAJAS ST.INFO (RECUADRO GENERAL) */
    ...
    /* 7. Optimización de márgenes superiores */
    .block-container {
...
def verificar_autenticacion():
    ...
    # Pase directo para iFrames en Moodle (Parámetros 'embed' o 'uner')
    qp = st.query_params
...
if verificar_autenticacion():
    ...
    *Desarrollado para la cátedra de Macroeconomía II. Este entorno interactivo permite analizar la microfundamentación 
    ...
    if modelo_seleccionado == "1. Modelo de 2 Períodos (Efectos Hicks y Trayectoria)":
        ...
        st.info(f"""
        **Intuición Económica para el Alumno:** Al incrementarse la tasa de interés de **{i_inicial*100:.0f}%** a **{i_final*100:.0f}%**, el consumo futuro se vuelve relativamente más barato, generando un **Efecto Sustitución Inequívocamente Negativo** de **{efecto_sustitucion:.2f}** unidades en el consumo presente ($C_1$).  
